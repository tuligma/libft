...
/*
	...
	*	Return:     	The uppercase character code if 'c' is a lowercase letter,
	*	            	'c' unchanged otherwise.
	*	External Functions:
	*	            	None.
	*	Walkthrough:
	*	            	1.	The function takes an integer 'c' as input.
	*	            	2.	It checks if 'c' is within the range of lowercase
	*	            		letters (a-z).
	*	            	3.	If 'c' is an lowercase letter, it subtracts 32 to
	*	            		'c' to convert it to the corresponding uppercase
	*	            		character code.
	*	            	4.	It returns the modified 'c' or the original 'c' if
	*	            		'c' is not an uppercase letter.
	*	Summary:
	*	            	The ft_toupper function converts an ASCII character code
	*	            	to its corresponding uppercase character code if the input
	*	            	character is an lowercase letter. If the input character
	*	            	is not a lowercase letter, it returns the input character
	*	            	unchanged.
	*	Usage:
	*	            	```c
	*	            	#include <stdio.h>
