...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/18 21:02:14 by npentini          #+#    #+#             */
/*   Updated: 2023/06/18 21:14:42 by npentini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		if (ft_isprint(c) != (isprint(c) != 0))
		{
			...
			printf("%sft_isprint%s is <%szero%s>.%s\n", YE, RE, YE, RE, CR);
		...
		else
		{
			...
			printf("\t\t\t\t\t=================================\n");
			printf("\t\t\t\t\t|  %sft_isprint%s   |    %sisprint	%s|",
				BU, CR, BU, CR);
			...
			if (ft_isprint(c) && isprint(c))
			{
				...
				printf("\t\t\t\t\t=================================\n");
				printf("\n");
				printf("%s \t\t\t\t\t\"%c\"%s %sis a printable character.%s\n",
					YE, c, CR, GR, CR);
			}
			else
			{
				...
				printf("\t\t\t\t\t=================================\n");
				printf("\n");
				printf("%s \t\t\t\t\t\"%c\"%s %sis not a printable character!%s\n",
					YE, c, CR, RE, CR);
			}
			printf("\n\n\n");
			printf("%sisprint():\n", GR);
			printf("\t%sDescription:\n", BU);
			printf("\t%schecks for any %sprintable ", YE, GR);
			printf("%scharacter including space.\n\n", YE);
